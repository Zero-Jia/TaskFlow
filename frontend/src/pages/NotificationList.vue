<template>
  <div class="page">
    ...
    <div class="container">
      ...
      <ErrorState v-if="errorMsg" title="通知加载失败" :message="errorMsg">
        <button class="btn secondary" @click="loadNotifications">重新加载</button>
      </ErrorState>

      <EmptyState
        v-if="!loading && !errorMsg && notifications.length === 0"
        title="暂无通知"
        description="当任务被分配或有新评论时，你会在这里看到提醒。"
      />
...
async function loadNotifications() {
  try {
    ...
    errorMsg.value = getErrorMessage(error, '获取通知失败')
  } finally {
    loading.value = false
  }
}
...
async function handleMarkRead(notificationId) {
  try {
    ...
    alert(getErrorMessage(error, '标记已读失败'))
  }
}
